...
export default function ProgressBar() {
  ...
  return (
    <div className="fixed top-4 right-4 w-64 z-50">
      <label className="block mb-1 text-sm font-semibold text-green-900">
        Progress
      </label>
      <input
        type="range"
        ...
        className="w-full accent-green-800 mb-2"
      />
      <div className="w-full h-6 bg-slate-700 rounded-full overflow-hidden">
        <motion.div
          animate={{ width: `${progress}%` }}
          transition={{ type: "spring", stiffness: 100, damping: 15 }}
          className="h-full bg-green-800 flex items-center justify-center text-xs font-semibold text-white"
        >
          {progress > 0 && `${progress}%`}
        </motion.div>
      </div>
